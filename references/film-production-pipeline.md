...
### 4.5 镜头技术标注（Lens & Technical）

#### 焦段选择
焦段决定画面视野和空间透视感：
- **14-24mm（广角）**：大视野、夸张透视、强调空间纵深 → 建立镜头/室内
- **35mm（小广角）**：略带环境的人像 → 纪录片风格/环境肖像
- **50mm（标准）**：最接近人眼 → 对话场景主力
- **85mm（中长焦）**：柔和透视+背景虚化 → 人像/特写
- **135mm+（长焦）**：压缩空间、平面感 → 远距离偷窥/隔阂感

#### 景深控制
- **Deep Focus（深焦 f/8~f/16）**：前后景都清晰，展示环境全貌
- **Shallow DOF（浅景深 f/1.4~f/2.8）**：仅主体清晰，隔离主体、情感聚焦
- **Rack Focus（移焦）**：焦点从A平滑移到B，引导注意力转移
- **Split Diopter（分焦镜）**：近景A+远景B同时清晰

### 4.6 180度规则与视线匹配

#### 180度规则（轴线法则）
对话/追逐场景中存在虚拟的**动作轴线**。摄影机必须保持在轴线同一侧：
- 角色A始终朝右看、角色B始终朝左看
- 越轴会破坏观众的空间方向感
- 必须越轴时用**中性镜头（正面/背面）**过渡

#### 视线匹配（Eyeline Match）
- 正反打镜头中A的视线方向 = B的视线反方向
- 高低差视线匹配：高个子角色俯视↔矮个子角色仰视
- 标注符号：`LOOK→/←/↑/↓/OFF-SCREEN/DIRECT-TO-CAMERA`

### 4.7 分镜板类型

| 类型 | 用途 | 阶段 |
|------|------|------|
| **Beat Board** | 情绪节拍板，每格=一个故事节拍 | 最早概念期 |
| **Color Script** | 全片色彩情绪走向（色块+简笔画） | 美术定调 |
| **Storyboard** | 逐镜头分镜板 | 前期制作 |
| **Technical Board** | VFX镜头技术标注（CGI/合成/跟踪） | VFX前期 |
| **Animatic** | 动态故事板（线稿+临时配音+时间轴） | 节奏验证 |
